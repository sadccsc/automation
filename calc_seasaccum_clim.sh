#!/bin/bash
#
# script to call python script that derives accumulated raifnall over rainy season
#
# P.Wolski
# June 2023
#
...
lstfile=$scriptdir/calc_seasaccum_clim.lst
...
if [ $# -ge 1 ]; then
    ...
    startdate=$(date +"%Y%m%d" -d "$enddate - 1 day")
fi
...
for item in ${indices[@]}; do
    ...
    while [ "$cdate" -le $enddate ]; do
        ...
        echo
        echo "*********************************"
        echo calling calc_seasaccum.py with $args
