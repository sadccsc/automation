#!/bin/bash
#
# script to call python script that derives pet
#
# P.Wolski
# June 2023
#
...
if [ $# == 0 ]; then
   ...
   echo Expected usage:
   echo calc_pet.sh lstfile.lst [startdate] [enddate] 
   exit
else
    lstfile=$1
    if [ ! -f $lstfile ]; then
...
for item in ${parameters[@]}; do
    ...
    var=${item[3]} #TG
    basetime=${item[4]} #mon
    index=${item[5]} #pet-har or pet-tho
    attribute=${item[6]} #index
    ...
    while [ "$cdate" -le $enddate ]; do
        ...
        echo
        echo "*********************************"
        echo calling calc_pet.py with $args


        echo python $scriptdir/calc_pet.py $indir $indexdir $args
        python3 $scriptdir/calc_pet.py $indir $indexdir $args
